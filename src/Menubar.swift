...
class Menubar {
    ...
    static func initialize() {
        ...
        menu.addItem(NSMenuItem.separator())
        addMenuItem(String(format: NSLocalizedString("About %@", comment: "Menubar option. %@ is AltTab"), App.displayName), #selector(App.showAboutWindow), "", "info.circle", nil, App.self)
        ...
        menu.addItem(NSMenuItem.separator())
        addMenuItem(String(format: NSLocalizedString("Quit %@", comment: "%@ is AltTab"), App.displayName), #selector(NSApplication.terminate(_:)), "q", nil) // "xmark.rectangle" is not necessary; macos automatically recognizes Quit
...
class PermissionCallout: StackView {
    ...
    static func message(_ dependentFeatures: PermissionCalloutResolver.DependentFeatures) -> String {
        ...
        return String(format: App.rebrand(NSLocalizedString("AltTab is running without Screen Recording permissions. %@ won’t show.", comment: "Menubar callout. %@ is one or more feature names, e.g. Thumbnails")), subject)
    }
}
